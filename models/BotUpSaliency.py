...
class BotUpSaliency(tf.keras.layers.Layer):
    ...
    def _build_interconnection(self):
        ...
        if self.verbose >= 2:
            self._save_multi_frame_from_multi_channel(W, "layers/video/WBotUp_inibition_filter.jpeg")
            self._save_multi_frame_from_multi_channel(J, "layers/video/JBotUp_exitatory_filter.jpeg")
    ...
    def _build_psi_kernel(self):
        ...
        if self.verbose >= 2:
            self._save_multi_frame_from_multi_channel(psi_kernel, "layers/video/PsiBotUp_filter.jpeg")
