...
run: python -m tests.NormBase.t11d_dynamic_holisitc_xy_pos
"""

# define configuration
config_path = 'NB_t11d_dynamic_holistic_xy_pos_m0003.json'
...
eyebrow_preds[:, :5] = 0  # hair
eyebrow_preds[:, 13:] = 0  # under eyes
eyebrow_preds[:, :, :7] = 0  # left side
eyebrow_preds[:, :, 21:] = 0  # right side
# for lips, remove all the top above the nose, and sides
lips_preds[:, :16] = 0  # hair
lips_preds[:, 26:] = 0  # under eyes
lips_preds[:, :, :8] = 0  # left side
lips_preds[:, :, 20:] = 0  # right side
...
# add holistic
# for eyebrow, remove all activity under the eye level and sides (ears)
test_eyebrow_preds[:, :2] = 0  # hair
test_eyebrow_preds[:, 9:] = 0  # under eyes
test_eyebrow_preds[:, :, :6] = 0  # left side
test_eyebrow_preds[:, :, 22:] = 0  # right side
# for lips, remove all the top above the nose, and sides
test_lips_preds[:, :16] = 0  # hair
test_lips_preds[:, 26:] = 0  # under eyes
test_lips_preds[:, :, :8] = 0  # left side
test_lips_preds[:, :, 20:] = 0  # right side

test_max_preds = np.concatenate([test_eyebrow_preds, test_lips_preds], axis=3)
print("test_max_preds", np.shape(test_max_preds))

# compute dynamic feature maps
# eyebrow
test_neutr_eyebrow_preds = get_ref_idx_frames([test_eyebrow_preds, data[1]], ref_index=config['ref_category'])
...
# lips
test_neutr_lips_preds = get_ref_idx_frames([test_lips_preds, data[1]], ref_index=config['ref_category'])
...
plot_cnn_output(preds, os.path.join("models/saved", config["config_name"]),
                "00_feature_maps_output.gif", verbose=True, video=True)
plot_cnn_output(dyn_preds, os.path.join("models/saved", config["config_name"]),
                "00_dyn_feature_maps_output.gif", verbose=True, video=True)
plot_cnn_output(test_preds, os.path.join("models/saved", config["config_name"]),
                "00_test_feature_maps_output.gif", verbose=True, video=True)
plot_cnn_output(test_dyn_preds, os.path.join("models/saved", config["config_name"]),
                "00_test_dyn_feature_maps_output.gif", verbose=True, video=True)
...
print("[PLOT] shape dyn_max_eyebrow_preds", np.shape(dyn_eyebrow_preds))
#
