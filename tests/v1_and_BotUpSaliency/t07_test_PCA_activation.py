...
from models.layers.layers import MaxPoolDepths
from tests.v1_and_BotUpSaliency.utils.unflatten_idx import unflatten_idx
import os
...
for f in range(np.shape(data)[0]):
    ...
    # save image
    cv2.imwrite("layers/video/seq_" + str(f) + ".jpeg", output.astype(np.uint8))
