...
def plot_cnn_output(output, path, name, title=None, image=None, video=False, verbose=True):
    """
    ...
    :return:
    """
    if verbose: print("start plot_cnn_output of", name)
    ...
    if not image is None:
        ...
        if np.max(image)>1:
            if verbose: print("transform image data to RGB [0..1] from [0..255]")
            image = image / 255
    ...
    update_list=[]
    def update(n_frame):
        if verbose and n_frame>=0: print("frame", n_frame)
        if not video:
            data = output
        else:
            data = output[n_frame,...]
    ...
    if not video:
        ...
        def init_func(): return []
        anim = FuncAnimation(fig, update, frames=output.shape[0], init_func=init_func, blit=True, interval=33)
        anim.save(os.path.join(path, name))
    if verbose: print("finished plot_cnn_output of", name)