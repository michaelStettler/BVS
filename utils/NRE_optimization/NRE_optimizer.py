...
from utils.NRE_optimization.NRE_loss import probability
...
def compute_NRE_preds(x, projections, radius, use_ref=False):
    ...
    if use_ref:
        prob = probability(x, projections, radius).numpy()
...
def transfer_domains(x_train, y_train, ref, trained_domain):
    # filter by domain that was trained
    idx_dom = tf.squeeze(tf.where(tf.math.logical_and(y_train[:, 0] == 0, y_train[:, 1] == trained_domain)))
    # idx_dom = tf.convert_to_tensor(idx_dom)
    x_dom = tf.gather(x_train, idx_dom)
    avg_neutral = tf.math.reduce_mean(x_dom, axis=0)
    # Get position of reference relative to the average neutral expression
    shift_from_avg = ref[trained_domain, :, :] - avg_neutral
    # Apply the shift to all other domains
    shifts_list = []
    for d in range(6):
        if d == trained_domain:
            shifts_list.append(tf.zeros((10, 2)))
        else:
            shifts_list.append(shift_from_avg)
    shifts_from_avg = tf.stack(shifts_list, axis=0)
    ref = ref + shifts_from_avg
    return ref
...
def fit_NRE(x, y, n_cat, x_test=None, y_test=None, use_ref=True, batch_size=32, n_ref=1,
            init_ref=None, lr=0.01, n_epochs=20,
            ...
            trained_domain=None, train_references=True):
    """
    ...
    for epoch in range(n_epochs):
        ...
        for x_batch, y_batch in batch(x, y, n=batch_size):
            ...
            if epoch == 0:
                starting_acc, _ = estimate_NRE(x_test, y_test,
        ...
        if trained_domain is not None:
            ref = transfer_domains(x, y, shifts, trained_domain=trained_domain)
        else:
            ref = shifts
        if is_test_set:
            epoch_test_acc, _ = estimate_NRE(x_test, y_test,
                                             {'references': ref, 'radius': radius,
                                              'tun_vectors': tun_vectors},
                                             n_ref=n_ref,
                                             batch_size=batch_size,
                                             use_ref=use_ref,
    ...
    if is_test_set:
        metrics = {'best_acc': best_acc, 'train_accuracies': train_accuracies, 'test_accuracies': test_accuracies,
                   'losses': losses, 'starting_acc': starting_acc}
    else:
