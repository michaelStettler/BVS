...
CNN_path = os.path.join(computer_path, 'model_behav_preds')
...
NRE_pred = NRE_pred[..., 1:]
NRE_pred = NRE_pred / np.sum(NRE_pred, axis=-1, keepdims=True)
...
print(np.sum(behav_data, axis=-1))
print(np.sum(NRE_pred, axis=-1))
print(np.sum(CNN_pred, axis=-1))
